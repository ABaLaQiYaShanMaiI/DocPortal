...
class App:
    ...
    def set_lang(self, lang):
        if lang != self._lang:
            ...
            saved_pout = (
                self.pout_var.get()
                if hasattr(self, "pout_var")
                else os.path.join(os.path.expanduser("~"), "Desktop", "knowledge_portal")
            )
            self._lang = lang
            self._save_settings()
            self.build_all()
    ...
    def _build_header(self):
        ...
        for lcode, ltxt in [("en", "EN"), ("zh", "中文")]:
            ...
            btn = tk.Button(
                ...
                pady=2,
                cursor="hand2",
                command=lambda c=lcode: self.set_lang(c),
            )
    ...
    def _build_file_list(self):
        ...
        self.search_hint_lbl = tk.Label(
            search_row, text="", font=("Segoe UI", 7), bg=self.COLORS["card"], fg=self.COLORS["text_secondary"]
        )
    ...
    def _build_settings(self):
        ...
        tk.Label(
            mode_f, text=self.tr("mode_label"), font=("Segoe UI", 10), bg=self.COLORS["card"], fg=self.COLORS["text"]
        ).pack(side=tk.LEFT)
        self.mode_var = tk.StringVar(value="single")
        for val, txt in [
            ("single", self.tr("single")),
            ("chunked", self.tr("chunked_mode")),
            ("portal", self.tr("portal_mode")),
        ]:
            rb = tk.Radiobutton(
                mode_f,
                text=txt,
        ...
        tk.Label(mode_f, text=f"({ps})", font=("Segoe UI", 9), bg=self.COLORS["card"], fg=pc).pack(
        ...
        tk.Label(
            r1, text=self.tr("output"), font=("Segoe UI", 10), bg=self.COLORS["card"], fg=self.COLORS["text"]
        ).pack(side=tk.LEFT)
        self.out_var = tk.StringVar(value=self.output_path)
        ttk.Entry(r1, textvariable=self.out_var, font=("Segoe UI", 9)).pack(
        ...
        out_btn = tk.Button(
            ...
            relief="flat",
            cursor="hand2",
            command=self.browse_txt_output,
            padx=10,
        )
        ...
        tk.Label(r2, text=self.tr("fname"), font=("Segoe UI", 10), bg=self.COLORS["card"], fg=self.COLORS["text"]).pack(
            side=tk.LEFT
        )
        ...
        tk.Label(r2, text="Format:", font=("Segoe UI", 10), bg=self.COLORS["card"], fg=self.COLORS["text"]).pack(
        ...
        for fmt_val, fmt_txt in [("txt", ".txt"), ("md", ".md")]:
            ...
            rb.pack(side=tk.LEFT, padx=(2, 0))
        self.format_ext_lbl = tk.Label(
            r2, text=".txt", font=("Segoe UI", 10, "bold"), bg=self.COLORS["card"], fg=self.COLORS["primary"]
        )
        self.format_ext_lbl.pack(side=tk.LEFT, padx=(4, 0))
        self.skip_var = tk.BooleanVar(value=True)
        tk.Checkbutton(
            self.single_f,
            text=self.tr("show_skip"),
        ...
        tk.Label(
            cr1, text=self.tr("out_dir"), font=("Segoe UI", 10), bg=self.COLORS["card"], fg=self.COLORS["text"]
        ).pack(side=tk.LEFT)
        self.chunk_out_var = tk.StringVar(value=os.path.join(os.path.expanduser("~"), "Desktop", "knowledge_chunked"))
        ttk.Entry(cr1, textvariable=self.chunk_out_var, font=("Segoe UI", 9)).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(4, 4)
        )
        chunk_out_btn = tk.Button(
            cr1,
            text=self.tr("browse"),
            font=("Segoe UI", 9),
            bg=self.COLORS["primary"],
            ...
            relief="flat",
            cursor="hand2",
            command=self.browse_chunked_out,
            padx=10,
        )
        ...
        tk.Label(
            cr2, text=self.tr("chunk_size_label"), font=("Segoe UI", 10), bg=self.COLORS["card"], fg=self.COLORS["text"]
        ).pack(side=tk.LEFT)
        ...
        tk.Label(
            pr1, text=self.tr("out_dir"), font=("Segoe UI", 10), bg=self.COLORS["card"], fg=self.COLORS["text"]
        ).pack(side=tk.LEFT)
        ...
        ttk.Entry(pr1, textvariable=self.pout_var, font=("Segoe UI", 9)).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(4, 4)
        )
        pout_btn = tk.Button(
            pr1,
            ...
            relief="flat",
            cursor="hand2",
            command=self.browse_portal_out,
            padx=10,
        )
        ...
        tk.Label(
            pr2, text=self.tr("port_label"), font=("Segoe UI", 10), bg=self.COLORS["card"], fg=self.COLORS["text"]
        ).pack(side=tk.LEFT)
    ...
    def _build_server_controls(self):
        ...
        self.server_stop_btn = tk.Button(
            ...
            relief="flat",
            cursor="hand2",
            command=self.server.stop,
            padx=10,
            pady=2,
        )
        self.server_copy_btn = tk.Button(
            self.server_f,
            text=f"\U0001f4cb {self.tr('copy_url')}",
            font=("Segoe UI", 9),
            bg=self.COLORS["primary"],
            fg="white",
            relief="flat",
            cursor="hand2",
            command=lambda: self.server.copy_url_to_clipboard(self.root),
            padx=10,
            pady=2,
        )
    ...
    def _update_server_ui(self):
        ...
        if self.server.is_running:
            self.server_status_lbl.config(
                text=f"\U0001f7e2 {self.tr('server_running')} {self.server.url}", fg=self.COLORS["success"]
            )
        ...
        if folder:
            ...
            file_path = filedialog.askopenfilename(
                ...
                filetypes=[
                    (
                        "All supported files",
                        "*.pdf *.docx *.doc *.txt *.md *.html *.py *.js *.ts *.json *.xml *.csv *.yaml *.yml",
                    ),
                    ("All files", "*.*"),
                ],
            )
    ...
    def _on_scanned_single(self, file_list, total_size):
        ...
        self.stats_lbl.config(
            text=f"1 {self.tr('files')} | {'1 ' + self.tr('supported') if supported else '0 supported'} | {human_readable_size(total_size)}"
        )
        self._update_tree()
    ...
    def _on_scanned(self, file_list, total_size):
        ...
        supported = sum(1 for f in file_list if f["supported"])
        self.stats_lbl.config(
            text=f"{len(file_list)} {self.tr('files')} | {supported} {self.tr('supported')} | {human_readable_size(total_size)}"
        )
        self._update_tree()
        if file_list:
            self.gen_btn.config(state="normal")
    ...
    def _gen_chunked(self):
        ...
        def task():
            try:
                result = write_chunks(
                    folder_path=self.current_folder, output_dir=out_dir, chunk_size=chunk_size, force_split=force_split
                )
    ...
    def _gen_portal(self, skip):
        ...
        def task():
            try:
                r = _get_portal()(
                    folder_path=self.current_folder, output_dir=out_dir, include_skipped=skip, language=self._lang
                )
    ...
    def _gen_done(self, out_path, parsed, skipped, errors, chars):
        ...
        st = (
            f"{self.tr('gen_done')} {parsed} files"
            + (f", {skipped} skipped" if skipped else "")
            + (f", {errors} errors" if errors else "")
            + f" | {chars:,} chars | {human_readable_size(fs)}"
        )
    ...
    def _portal_done(self, result):
        ...
        st = (
            f"{self.tr('portal_done')} {dc} files"
    ...
    def _filter_file_tree(self):
        ...
        if not query:
            ...
            self.stats_lbl.config(
                text=f"{len(self.file_list)} {self.tr('files')} | {sum(1 for f in self.file_list if f['supported'])} {self.tr('supported')} | {human_readable_size(self.total_size)}"
            )
            return
        ...
        for i, item in enumerate(self.tree.get_children()):
            ...
            if mode == "name":
                ...
                match = query in clean_name.lower() or (
                    i in self._content_search_index and query in self._content_search_index[i]
                )
            if match:
    ...
    def _on_tree_double_click(self, event):
        ...
        if not selection:
            return
        clean_name = (
            self.tree.item(selection[0], "values")[0].replace("\U0001f4c4", "").replace("\u23ed\ufe0f", "").strip()
        )
